...
function filterByTagRules(targets, actionKey) {
  if (!actionKey) return targets;
  return targets;
}
...
class BattleSystem {
  ...
  static fromCreatePayload(playerUnits, enemyUnits, placement) {
    const sys = new BattleSystem({ combatants: [], round: 1, log: [], done: false, winner: null });
    sys.initCombatants(playerUnits, enemyUnits, placement);
    return sys;
  }
  ...
  applyOnHitReceivedPassives(actor, target, dmg) {
    ...
    if (passive.startsWith('volcanic_skin')) {
      actor.battle_hp = Math.max(0, actor.battle_hp - 12);
      if (actor.battle_hp <= 0) { actor.alive = false; this.applyOnDeathPassives(actor); }
      this.pushLog({ type: 'passive', passive: 'Volcanic Skin', actorName: target.unit_name, actorCell: target.cellIndex, targetName: actor.unit_name, targetCell: actor.cellIndex, value: 12, heal: false });
    }
  ...
  runEnemyTurns() {
    let safety = 0;
    while (!this.done && this.currentActor()?.side === 'enemy' && safety < 20) {
      this.aiTurn();
      safety++;
    }
  }
...
module.exports = { BattleSystem, cellIndex, cellRow, cellCol };