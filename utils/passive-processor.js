...
function executeActiveAbility(actor, target, combatants, UNIT_ABILITIES, engine) {
  ...
  if (p.stone_form_armor != null) {
    let armorAmt = p.stone_form_armor;
  ...
  // ── Ember Shroud ──────────────────────────────────────────────────────────
  // Frost Armor's resistance half plus Stone Form's mend, and no armor at all —
  // this is for a caster whose armor is zero and whose problem is fire.
  if (p.ember_shroud_resist != null) {
    const school = p.ember_shroud_resist_type || 'fire';
    // A re-cast refreshes rather than adds: only one amount can be handed back,
    // so stacking would leak the first grant. Same reasoning as Frost Armor.
    if (actor._ember_shroud_rounds > 0) engine.expireEmberShroud(actor);

    const resistAmt = addResist(actor, school, p.ember_shroud_resist);
    actor._ember_shroud_rounds = p.duration_rounds ?? 2;
    actor._ember_shroud_resist = resistAmt;
    actor._ember_shroud_school = school;

    const resPath = actor.unit_data?.resistances ? 'unit_data.resistances' : 'resistances';
    engine.registerEffect(actor, {
      key: 'ember_shroud', name: def.name, polarity: 'positive', dispellable: def.dispellable === true,
      restore: { [`${resPath}.${school}`]: -resistAmt },
      clear:   { _ember_shroud_rounds: 0, _ember_shroud_resist: 0, _ember_shroud_school: null },
    });

    const pct    = p.ember_shroud_heal_pct ?? 0;
    const factor = 1 - ((actor._healing_reduction ?? 0) / 100);
    const healed = Math.min(
      engine.absorbWithDecay(actor, Math.floor(actor.max_hp * pct / 100 * factor * engine.fatigueHealMult())),
      actor.max_hp - actor.battle_hp);
    ...
    engine.pushLog({ type: 'ability', ability: abilityKey, actorId: actor.id, actorName: actor.unit_name, actorCell: actor.cellIndex,
      targetId: actor.id, targetName: actor.unit_name, targetCell: actor.cellIndex,
      value: healed, heal: true,
      message: `${def.name} — +${resistAmt} ${school} resist for ${p.duration_rounds ?? 2} rounds, mended ${healed}` });
    if (healed > 0) engine.fireHealTriggers(actor, actor, healed);
  }

  // ── Dragon's Breath ───────────────────────────────────────────────────────
  // One target is picked; the breath takes the ROW that target stands in. The
  // row is read off footprints, not the anchor cell, so a 'row' or 'column'
  // unit is caught by any of the cells it actually occupies.
  if (p.row_damage_per_tag != null && target) {
    const mainTags  = tagCount(engine, actor.side, p.tag_required);
    const bonusTags = p.bonus_tag_required ? tagCount(engine, actor.side, p.bonus_tag_required) : 0;
    const raw = p.row_damage_per_tag * mainTags + (p.row_damage_bonus_per_tag ?? 0) * bonusTags;
    const row = cellRow(target.cellIndex);
    if (raw <= 0) {
      engine.pushLog({ type: 'ability', ability: abilityKey, actorId: actor.id, actorName: actor.unit_name, actorCell: actor.cellIndex,
        targetId: target.id, targetName: target.unit_name, targetCell: target.cellIndex,
        message: `${def.name} — no ${p.tag_required} on the field; the breath guts out` });
    } else {
      const victims = combatants
        .filter(c => c.side !== actor.side && c.alive && !c._untargetable && !c._invulnerable &&
                     engine.getFootprint(c).some(cell => cellRow(cell) === row))
        .sort((a, b) => cellCol(a.cellIndex) - cellCol(b.cellIndex));
      for (const v of victims) {
        const dmg = typedAmount(v, raw, p.damage_type);
        hurt(v, dmg);
        const dead = v.battle_hp <= 0;
        if (dead) { v.alive = false; engine.applyOnDeathPassives(v); }
        engine.pushLog({ type: 'ability', ability: abilityKey, actorId: actor.id, actorName: actor.unit_name, actorCell: actor.cellIndex,
          targetId: v.id, targetName: v.unit_name, targetCell: v.cellIndex,
          value: dmg, heal: false, killed: dead,
          message: `${def.name} — scorched ${v.unit_name} for ${dmg}` });
      }
    }
  }

  // ── Verse of Cataclysm ────────────────────────────────────────────────────
  // The damage lands FIRST and the singer dies after, through the ordinary death
  // path — so Last Verse and every other on-death passive fire on top of this
  // rather than instead of it. That stacking is the reason the ability exists.
  if (p.self_kill) {
    const enemies = combatants.filter(c => c.side !== actor.side && c.alive && !c._untargetable && !c._invulnerable);
    for (const e of enemies) {
      const dmg = typedAmount(e, p.cataclysm_damage ?? 0, p.damage_type);
      hurt(e, dmg);
      const dead = e.battle_hp <= 0;
      if (dead) { e.alive = false; engine.applyOnDeathPassives(e); }
      engine.pushLog({ type: 'ability', ability: abilityKey, actorId: actor.id, actorName: actor.unit_name, actorCell: actor.cellIndex,
        targetId: e.id, targetName: e.unit_name, targetCell: e.cellIndex,
        value: dmg, heal: false, killed: dead,
        message: `${def.name} — ${e.unit_name} takes ${dmg}` });
    }
    if (actor.alive) {
      actor.battle_hp = 0;
      actor.alive = false;
      engine.pushLog({ type: 'ability', ability: abilityKey, actorId: actor.id, actorName: actor.unit_name, actorCell: actor.cellIndex,
        targetId: actor.id, targetName: actor.unit_name, targetCell: actor.cellIndex,
        value: 0, heal: false, killed: true,
        message: `${def.name} — ${actor.unit_name} sings itself apart` });
      engine.applyOnDeathPassives(actor);
    }
  }

  // ── Blessing of Protection ────────────────────────────────────────────────
  // The rounds counter is what resolveProtectorIntercept reads; the armor is
  // held on the unit so expireGuard can hand back exactly what was applied.
  if (p.guaranteed_intercept && target) {
    if (target._guard_rounds > 0) engine.expireGuard(target);

    const n = tagCount(engine, actor.side, p.tag_required);
    const armorAmt = addArmor(target, (p.armor_per_tag ?? 0) * n);
    target._guard_rounds = p.duration_rounds ?? 2;
    target._guard_armor  = armorAmt;

    engine.registerEffect(target, {
      key: 'guard', name: def.name, polarity: 'positive', dispellable: def.dispellable === true,
      restore: { armor: -armorAmt },
      clear:   { _guard_rounds: 0, _guard_armor: 0 },
    });
    if (armorAmt > 0) engine.recordGrantedBuff(actor, 'armor', [target], armorAmt);

    engine.pushLog({ type: 'ability', ability: abilityKey, actorId: actor.id, actorName: actor.unit_name, actorCell: actor.cellIndex,
      targetId: target.id, targetName: target.unit_name, targetCell: target.cellIndex,
      value: armorAmt, heal: false, stat: 'armor',
      message: `${def.name} — ${target.unit_name} intercepts every blow for ${p.duration_rounds ?? 2} rounds, +${armorAmt} armor (${n} ${p.tag_required})` });
  }
